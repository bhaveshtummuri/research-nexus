 ...
 * API contract types.
 *
 * Mirrored from `server/src/types/domain.ts`. The two packages ship
 * independently, so the contract is duplicated rather than imported across the
 * workspace boundary - the client can be deployed against any server that
 * honours these shapes.
 */
...
export interface HiddenCollaborator extends ScoredAuthor {
  ...
  distance: number;
  /** Work actually co-authored. Always empty for hidden collaborators. */
...
export interface CollaborativeResearcher extends AuthorSummary {
  ...
  jointPapers: number;
  score: number;
  topPartners: Array<Pick<AuthorSummary, 'id' | 'name'>>;
}

/**
 * One node of the citation tree. The API returns the tree flat; `parentId`
 * rebuilds the hierarchy in a single pass on the client.
 */
...
export interface CitationTreeMeta {
  count: number;
  rootId: string;
...
export interface TopicFieldSummary {
  field: string;
  topicCount: number;
  paperCount: number;
}
